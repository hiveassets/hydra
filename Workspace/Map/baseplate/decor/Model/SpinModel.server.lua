--[[
    SpinModel (Script)
    Path: Workspace → Map → baseplate → decor → Model
    Parent: Model
    Properties:
        Disabled: false
...
local baseSpeed=math.rad(-0.6)
local boostAmount=math.rad(-2)
...
r.Heartbeat:Connect(function(d)
	...
	if elapsed<1.5 then
		...
		m:PivotTo(m:GetPivot()*CFrame.Angles(0,(baseSpeed+boost*(1-t)^2)*d*scale,0))
		if t>=1 then boost=0 end
	else
		m:PivotTo(m:GetPivot()*CFrame.Angles(0,baseSpeed*d*scale,0))
	end
end)